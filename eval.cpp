...
int process_builtin_command(string line) {
    ...
    // 1 - cd
    if (line == "cd") {
        return builtins::process_line(line);
    }
    // 2 - help
    else if (line == "help"){
        return builtins::help::process_line(line);
    }
    // 3 - alias
    // 3.5 - unalias
    // 4 - history
    else if (line == "history") {
        return builtins::history::process_line(line);
    }
    // 5 - type

    // 6 - exit
    else if (line == "exit") {
        return builtins::exit::process_line(line);
    }
    // 下面几个命令没有要求实现
    // 7 - jobs
    // 8 - job_spec
